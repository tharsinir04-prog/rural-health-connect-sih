...
<p>
One connected workflow for registration,
longitudinal records, appointments,
...
<label>Purpose *</label>
<input id="fPurpose"
placeholder="BP / diabetes / maternal / child">
</div>
...
.textContent=d.patients;

document.getElementById('sAppointments')
.textContent=d.appointments;

document.getElementById('sReferrals')
.textContent=d.referrals;

document.getElementById('sFollowups')
.textContent=d.follow_ups;
...
.innerHTML=d.patients.map(function(p){

return '<div class="patient-card">'+
'<h3>'+escapeHtml(p.name)+'</h3>'+
'<p><b>Patient ID:</b> '+escapeHtml(p.id)+'</p>'+
'<p><b>Age:</b> '+escapeHtml(p.age||'N/A')+'</p>'+
'<p><b>Gender:</b> '+escapeHtml(p.gender||'N/A')+'</p>'+
'<p><b>Village:</b> '+escapeHtml(p.village||'N/A')+'</p>'+
'<p><b>Phone:</b> '+escapeHtml(p.phone||'N/A')+'</p>'+
'<button class="primary" onclick="selectPatient('+Number(p.id)+')">'+
...
msg('patientResults',e.message,'error');
...
var id=document.getElementById('recordPatientId')
.value.trim();
...
'<p><b>ID:</b> '+escapeHtml(p.id)+
' | <b>Age:</b> '+escapeHtml(p.age||'N/A')+
' | <b>Gender:</b> '+escapeHtml(p.gender||'N/A')+
'</p>'+
'<p><b>Village:</b> '+escapeHtml(p.village||'N/A')+
' | <b>Phone:</b> '+escapeHtml(p.phone||'N/A')+
'</p>'+
...
'<b>'+escapeHtml(x.record_type||'Record')+'</b>'+
'<div>'+escapeHtml(x.notes)+'</div>'+
'<div class="muted">'+
...
' · '+
escapeHtml(x.doctor_name||'Doctor not assigned')+
' · <span class="pill">'+
escapeHtml(x.status)+
'</span></div>';
...
document.getElementById('fullRecord')
.innerHTML=html;

}catch(e){

msg('fullRecord',e.message,'error');
...
'/api/medical-records',
{
patient_id:document.getElementById('mPatientId').value,
recorded_by:document.getElementById('mRecordedBy').value,
record_type:document.getElementById('mType').value,
...
doctor_name:document.getElementById('aDoctor').value,
...
.innerHTML=d.appointments.length?
...
escapeHtml(x.appointment_time)+
' · '+
escapeHtml(x.doctor_name||'Unassigned')+
' · <span class="pill">'+
escapeHtml(x.status)+
'</span><br>'+
'<span class="muted">'+
escapeHtml(x.reason||'')+
'</span><br>'+
'<button class="smallbtn ok" onclick="appointmentStatus('+
x.id+
',\'completed\')">Complete</button>'+
'<button class="smallbtn warn" onclick="appointmentStatus('+
x.id+
...
'/api/referrals',
{
patient_id:document.getElementById('rPatientId').value,
from_facility:document.getElementById('rFrom').value,
to_facility:document.getElementById('rTo').value,
...
escapeHtml(x.status)+
'</span><br>'+
'<button class="smallbtn ok" onclick="referralStatus('+
x.id+
',\'completed\')">'+
...
'/api/follow-ups',
{
patient_id:document.getElementById('fPatientId').value,
follow_up_date:document.getElementById('fDate').value,
